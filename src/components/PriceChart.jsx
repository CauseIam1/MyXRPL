...
const PriceChart = ({
  ...
  const processChartData = (mergedData) => {
    ...
    const interval = 3600;
    const hourlyBuckets = {};
    const dataStartTime = Math.min(
      ...filteredTransactions.map((tx) => tx.time)
    );
    const dataEndTime = Math.max(...filteredTransactions.map((tx) => tx.time));
    for (let time = dataStartTime; time <= dataEndTime; time += interval) {
  ...
  const calculateCurrentRate = (pair, tokenPrices, isAsset1Higher) => {
    try {
      const priceKey1 = `${pair.currency1}-${pair.issuer1}`;
      const priceKey2 = `${pair.currency2}-${pair.issuer2}`;
  ...
  const loadChartData = async (pair) => {
    ...
    try {
      ...
      if (poolResult) {
        ...
        if (ammTransactions && ammTransactions.length > 0) {
          ...
          if (ammData.length > 0) {
            ...
            const processedData = processChartData(ammData);
            let enhancedData =
              processedData.length > 0 ? processedData : ammData.slice(0, 50);
            if (enhancedData.length > 0 && enhancedData.length <= 5) {
            ...
            source = finalData.some((point) => point.isCurrentRate)
              ? enhancedData.length > processedData.length
                ? "enhanced"
                : "amm"
              : "amm";
          }
      ...
      setProgress({ message: "", percent: 0 });
      try {
        const fallbackData = getCurrentRateOnlyData(pair, tokenPrices);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span>
            📈 Tokens of {quoteCurrency} for 1 {baseCurrency}
          </span>
        </div>
        <div
          style={{
      ...
      >
        ...
        ) : error && dataSource !== "current" && dataSource !== "none" ? (
          <div
            style={{
              height: "100%",
              display: "flex",
      ...
      >
        ...
        {dataSource === "enhanced" && (
          <div style={{ marginTop: "2px", color: "#fbbf24" }}>
            ⚠️ Enhanced with current exchange rate
          </div>
        )}
        {dataSource === "current" && (
          <div style={{ marginTop: "2px", color: "#fbbf24" }}>
        ...
        {dataSource === "none" && (
          <div style={{ marginTop: "2px", color: "#fbbf24" }}>
            ⚠️ No recent trading data available
          </div>
        )}
      </div>
    </div>
