...
const PairsChart = ({
  ...
  const loadChartData = useCallback(async () => {
    ...
    try {
      ...
      const {
        ammData,
        asset1IsBase: determinedAsset1IsBase,
        fromCache,
      } = result;
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      ...
      const volumeBase = data.volume / data.quotePerBase;
      const arrow = data.isBuyingBase ? "↑" : "↓";
      const arrowColor = data.isBuyingBase
        ...
        >
          ...
          <div>
            ...
            <span style={{ color: themes[theme].textSecondary }}>
              {formatNumber(volumeBase, 4)}
            </span>
          </div>
          <div style={{ textAlign: "center", margin: "4px 0" }}>
            <span style={{ color: arrowColor, fontSize: "1.2rem" }}>
              {arrow}
            </span>
          </div>
          <div>
            Volume ({quoteCurrency}):{" "}
            <span style={{ color: themes[theme].textSecondary }}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          {["1H", "6H", "24H", "7D", "ALL"].map((range) => (
            <button
              key={range}
              onClick={() => setTimeRange(range === "ALL" ? "30D" : range)}
              style={{
                background:
                  timeRange === range ||
                  (timeRange === "30D" && range === "ALL")
                    ? themes[theme].primary
                    : themes[theme].glass,
                color:
                  timeRange === range ||
