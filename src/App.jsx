...
function App() {
  ...
  // Calculate visible pairs count for consistent use across components
    ...
    const handleTokenSelect = (token) => {
      if (
        ...
        if (tokenPrices[priceKey] !== undefined) {
          ...
          setShowPairChart(false); // Switch back to token chart mode
        } else {
          setTimeout(() => {
            setSelectedToken(token);
            setShowPairChart(false); // Switch back to token chart mode
  ...
  const handleLogout = () => {
    ...
    setSelectedToken(null); // Clear selected token on logout
    if (wsRef.current) {
      wsRef.current.close();
    }
    if (xrplClient) {
  ...
  * {
  ...
  /* Flip switch styling */
  #flip-switch:checked + label {
    background: linear-gradient(135deg, #4ade80, #16a34a);
  }
  
  #flip-switch:checked + label span {
                      ...
                      >
                        {showPairChart && selectedPair ? (
                          ...
                          <>
                            ...
                            >
                              <span>📈 Tokens per XRP (Inverse of Price)</span>
                              <span
                                style={{
                                  fontSize: "1.5rem",
                                  color: themes[theme].textSecondary,
