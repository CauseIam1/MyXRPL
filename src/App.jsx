...
const MIN_SEARCH_LEDGER = 98500589;
...
function App() {
  ...
  const [loginLedger, setLoginLedger] = useState(MIN_SEARCH_LEDGER);
  ...
  const getSwapGroups = () => {
    ...
    const MAX_TRANSACTIONS_PER_PAIR = 100;
    const MAX_TOTAL_TRANSACTIONS = 5000;
    const groupedSwaps = {};
    let totalTransactionsCount = 0;

    transactions.forEach((swap) => {
      if (totalTransactionsCount >= MAX_TOTAL_TRANSACTIONS) return;
      ...
      if (groupedSwaps[pairKey].swaps.length < MAX_TRANSACTIONS_PER_PAIR) {
  ...
  const trackWalletLogin = (walletAddress) => {
    try {
      ...
      existingStats[walletAddress].loginLedger =
        ledgerIndex !== "Loading..." ? ledgerIndex : MIN_SEARCH_LEDGER;
  ...
  const updateCachedSwapGroups = async (account, newTransactions) => {
    ...
    try {
      ...
      uniqueSwaps.forEach((swap) => {
        ...
        if (swapGroups[pairKey].swaps.length < MAX_TRANSACTIONS_PER_PAIR) {
      ...
      const swapCacheData = {
        ...
        lastLedger:
          ledgerIndex === "Loading..." ? MIN_SEARCH_LEDGER : ledgerIndex,
        timestamp: Date.now(),
        account: account,
      };
  ...
  const loadTransactions = async (overrideFilterSinceLogin = null) => {
    ...
    try {
      ...
      let lastSwapUpdateLedger = MIN_SEARCH_LEDGER;
      if (cachedSwapData) {
        try {
          ...
          lastSwapUpdateLedger = parsedCache.lastLedger || MIN_SEARCH_LEDGER;
        } catch (e) {
      ...
      let lastFullLoadLedger = MIN_SEARCH_LEDGER;
      ...
      if (cachedTxData) {
        try {
          ...
          lastFullLoadLedger = parsedCache.lastLedger || MIN_SEARCH_LEDGER;
        } catch (e) {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.onopen = () => {
        const request = {
          ...
          command: "account_tx",
          account: account,
          ledger_index_min: MIN_SEARCH_LEDGER,
          ledger_index_max: -1,
          limit: 400,
          forward: true,
        };
  ...
  useEffect(() => {
    if (
      ...
      const MAX_TRANSACTIONS_PER_PAIR = 500;
      const groupedSwaps = {};
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      if (accountAddress && ledgerIndex !== "Loading..." && isLoggedIn) {
        try {
          const minLedger = loginLedger;
          const maxLedger = ledgerIndex;
          console.log(
            `Fetching transactions from ledger ${minLedger} to ${maxLedger}`
          );
          if (minLedger < maxLedger) {
            ...
            if (newTransactions.length > 0) {
              ...
              if (updatedGroups) {
                setTransactions(updatedGroups.flatMap((group) => group.swaps));
              }
            } else {
