  ...
  const updateCachedSwapGroups = async (account, newTransactions) => {
    ...
    try {
      ...
      const updatedCacheData = {
        ...
        lastLedger:
          ledgerIndex === "Loading..." ? parsedCache.lastLedger : ledgerIndex,
        timestamp: Date.now(),
        account: account,
      };

      localStorage.setItem(swapCacheKey, JSON.stringify(updatedCacheData));
      console.log("Updated cached swap groups with new data");
    ...
    try {
      ...
      if (cachedSwapData) {
        try {
          ...
          console.log("Loaded cached swap groups:", cachedSwapGroups.length);
        } catch (e) {
      ...
      if (
        ...
        try {
          ...
          console.log("Cached swap groups:", groupedSwaps.length);
  ...
  const cleanupOldCache = () => {
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (
          key &&
          (key.startsWith(CACHE_KEY_PREFIX) || key.startsWith("swap_groups_"))
        ) {
          try {
            const cachedData = JSON.parse(localStorage.getItem(key));
            if (cachedData.timestamp < oneWeekAgo) {
        ...
        try {
          ...
          if (cachedData) {
            ...
            if (lastLedger < ledgerIndex) {
              console.log(`Fetching transactions since ledger ${lastLedger}`);
              const newTransactions = await fetchTransactionsSince(
                accountAddress,
                lastLedger,
                ledgerIndex
